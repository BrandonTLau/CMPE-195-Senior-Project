...
function login(email, password) {
  ...
  if (!ok) { authFailures.add(1); errorRate.add(1); return null; }
  errorRate.add(0);
...
function registerUser() {
  ...
  const res = http.post(`${BASE_URL}/api/auth/register`,
    JSON.stringify({ fullName: 'Stress User', email, password: 'stresspass123' }),
...
export function handleSummary(data) {
  return {
    'backend-stress-summary.json': JSON.stringify(data, null, 2),
