...
// ─────────────────────────────────────────────────────────────
// INLINE TEXT EDITOR
// Select any text → "✎ Make a quick edit" tooltip appears →
// click to open an inline textarea pre-filled with that snippet.
// On save the snippet is spliced back into the full string and
// the parent's setter is called — NO backend calls.
// ─────────────────────────────────────────────────────────────
function InlineEditableText({ text, onSave }) {
  ...
  // Splice edited snippet back into full text and call parent setter
  const commitEdit = () => {
    if (!editValue.trim()) return;
    const updated = text.slice(0, selection.start) + editValue + text.slice(selection.end);
    onSave(updated);   // parent receives the complete updated string
    setIsEditing(false);
  };
...
const ResultsPage = ({ onBack }) => {
  ...
  // ── Static demo content (replaces backend fetch) ──────────
  ...
  const [aiSummary, setAiSummary] = useState(
    `Machine learning is a branch of AI focused on developing algorithms that allow computers to learn from experience. It includes three main types: supervised learning (using labeled data), unsupervised learning (finding patterns), and reinforcement learning (trial and error). Common applications are image recognition, NLP, and predictive analytics.`
  );
  const [studyGuideText, setStudyGuideText] = useState('');        // kept for future use
  const [transcriptionEdited, setTranscriptionEdited] = useState(false);
  const [summaryEdited,       setSummaryEdited]       = useState(false);

  // ── BACKEND COMMUNICATION — ALL COMMENTED OUT ─────────────
  /*
  ...
  */
  ...
  const handleSaveNotes = () => {
    setIsSaved(true);
    console.log('Notes saved locally (backend disabled).');
  };

  const handleExport = (format) => {
    console.log(`Export as ${format} — backend disabled.`);
  };

  const handleTranscriptionSave = (newText) => {
    setRecognizedText(newText);
    setTranscriptionEdited(true);
  };

  const handleSummarySave = (newText) => {
    setAiSummary(newText);
    setSummaryEdited(true);
  };

  // Regenerate buttons kept visible but no-op while backend is off
  const handleRegenerateAll = () => {
    console.log('Regenerate all — backend disabled.');
    // requestRegenerate('all');   // ← re-enable when backend is live
  };

  const handleRegenerateSummary = () => {
    console.log('Regenerate summary — backend disabled.');
    // requestRegenerate('summary');  // ← re-enable when backend is live
  };
    ...
    <div style={styles.container}>
      <div style={styles.wrapper}>
        ...
        <div style={styles.contentGrid}>
          ...
          {/* Recognized text card with inline editor */}
          <div style={styles.card}>
            <h3 style={styles.cardTitle}>
              <svg style={styles.cardIcon} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            ...
            {/* "Regenerate" shown after a local edit; calls no-op until backend re-enabled */}
            {transcriptionEdited && (
          ...
          <div style={styles.aiGrid}>
            ...
            <div style={styles.aiCard}>
              <div style={styles.aiCardHeader}>
                ...
                {/* Regenerate button — no-op while backend is off */}
                <button style={styles.regenerateButton} onClick={handleRegenerateSummary}>
