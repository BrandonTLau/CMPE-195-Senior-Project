...
const api = {
  ...
  toggleFavorite: async (id, current) => {
    const res = await fetch(`/api/files/${id}/meta`, { method:'PATCH', headers:{ 'Content-Type':'application/json', 'x-auth-token':getToken()||'' }, body:JSON.stringify({ isFavorite:!current }) });
    return handleRes(res);
  },
  updateTags: async (id, tags) => {
    const res = await fetch(`/api/files/${id}/meta`, { method:'PATCH', headers:{ 'Content-Type':'application/json', 'x-auth-token':getToken()||'' }, body:JSON.stringify({ tags }) });
    return handleRes(res);
  },
  moveNote: async (id, folderId) => {
    const res = await fetch(`/api/files/${id}/meta`, { method:'PATCH', headers:{ 'Content-Type':'application/json', 'x-auth-token':getToken()||'' }, body:JSON.stringify({ folderId }) });
    return handleRes(res);
  },
  permanentDelete: async (id) => {
    const res = await fetch(`/api/files/${id}`, { method:'DELETE', headers:{ 'x-auth-token':getToken()||'' } });
    return handleRes(res);
  },
  trashNote: async (id) => {
    const res = await fetch(`/api/files/${id}/meta`, { method:'PATCH', headers:{ 'Content-Type':'application/json', 'x-auth-token':getToken()||'' }, body:JSON.stringify({ isDeleted:true }) });
    return handleRes(res);
  },
  restoreNote: async (id) => {
    const res = await fetch(`/api/files/${id}/meta`, { method:'PATCH', headers:{ 'Content-Type':'application/json', 'x-auth-token':getToken()||'' }, body:JSON.stringify({ isDeleted:false }) });
    return handleRes(res);
  },
  emptyTrash: async (trashedIds) => {
    const results = await Promise.all(trashedIds.map(id =>
      fetch(`/api/files/${id}`, { method:'DELETE', headers:{ 'x-auth-token':getToken()||'' } }).then(handleRes).catch((err) => ({ err, id }))
    ));
  ...
  getFolders: async () => {
    const res = await fetch('/api/folders', { headers:{ 'x-auth-token':getToken()||'' } });
    return handleRes(res);
  },
  createFolder: async (name) => {
    const res = await fetch('/api/folders', { method:'POST', headers:{ 'Content-Type':'application/json', 'x-auth-token':getToken()||'' }, body:JSON.stringify({ name }) });
    return handleRes(res);
  },
  deleteFolder: async (id) => {
    const res = await fetch(`/api/folders/${id}`, { method:'DELETE', headers:{ 'x-auth-token':getToken()||'' } });
    return handleRes(res);
  },
  renameFolder: async (id, name) => {
    const res = await fetch(`/api/folders/${id}`, { method:'PATCH', headers:{ 'Content-Type':'application/json', 'x-auth-token':getToken()||'' }, body:JSON.stringify({ name }) });
    return handleRes(res);
  },
  changePassword: async (currentPassword, newPassword) => {
    const res = await fetch('/api/auth/password', { method:'PUT', headers:{ 'Content-Type':'application/json', 'x-auth-token':getToken()||'' }, body:JSON.stringify({ currentPassword, newPassword }) });
    return handleRes(res);
  },
  deleteAccount: async () => {
    const res = await fetch('/api/auth/account', { method:'DELETE', headers:{ 'x-auth-token':getToken()||'' } });
    return handleRes(res);
  },
};
      ...
      <div className="ud-modal" onClick={e => e.stopPropagation()} style={{ width:340 }}>
        ...
        {folders.length === 0 ? (
          <div style={{ textAlign:'center', padding:'16px 0 8px' }}>
            ...
            <p style={{ fontFamily:T.font, fontSize:13, color:T.muted, marginBottom:16, maxWidth:'26ch', margin:'0 auto 16px' }}>No folders yet.</p>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:2 }}>
            {currentFolderId && (<><button className="ud-dot-item" onClick={() => { onMove(null); onClose(); }}><Icon d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" size={14} />All Notes (remove from folder)</button><div className="ud-dot-sep" /></>)}
            {folders.map(folder => {
...
const FoldersStrip = ({ folders, notes, selectedFolderId, onSelect, onAdd, onDelete, onRename, onDropNote, search='' }) => {
  ...
  const handleDrop = (e, fid) => { e.preventDefault(); const nid = e.dataTransfer.getData('noteId'); if (nid) onDropNote(nid, fid); setDragOverId(undefined); };
  return (
      ...
      {!collapsed && (
        <>
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            <button className={`ud-folder-chip${selectedFolderId===null?' active':''}${draggingNoteId&&dragOverId===null?' dragover':''}`} onClick={() => onSelect(null)} onDragOver={e => handleDragOver(e,null)} onDragLeave={() => setDragOverId(undefined)} onDrop={e => handleDrop(e,null)}>
            ...
            {visible.map(folder => {
              ...
              return renamingId===folder.id ? (
                <input key={folder.id} className="ud-input" autoFocus value={renameVal} onChange={e => setRenameVal(e.target.value)} style={{ width:120, padding:'5px 10px', fontSize:12 }} onKeyDown={e => { if (e.key==='Enter'&&renameVal.trim()) { onRename(folder.id,renameVal.trim()); setRenamingId(null); } if (e.key==='Escape') setRenamingId(null); }} onBlur={() => setRenamingId(null)} />
              ) : (
                <div key={folder.id} style={{ position:'relative', display:'inline-flex' }}>
                  <button className={`ud-folder-chip${isActive?' active':''}${isDragOver?' dragover':''}`} onClick={() => onSelect(folder.id)} onDragOver={e => handleDragOver(e,folder.id)} onDragLeave={() => setDragOverId(undefined)} onDrop={e => handleDrop(e,folder.id)} onDoubleClick={() => { setRenamingId(folder.id); setRenameVal(folder.name); }}>
            ...
            <button onClick={onAdd} title="New folder" style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', width:38, height:34, borderRadius:8, border:`1px dashed rgba(255,255,255,0.18)`, background:'transparent', cursor:'pointer', color:T.muted, transition:'border-color .15s, color .15s', flexShrink:0 }} onMouseEnter={e => { e.currentTarget.style.borderColor='rgba(245,166,35,.4)'; e.currentTarget.style.color=T.amber; }} onMouseLeave={e => { e.currentTarget.style.borderColor='rgba(255,255,255,0.18)'; e.currentTarget.style.color=T.muted; }}>
...
const NoteCard = ({ note, onOpen, onToggleFavorite, onDelete, onUpdateTags, folders=[], onMoveNote, onCreateFolder, selectionMode=false, selected=false, onToggleSelect, onEnterSelectionMode }) => {
  ...
  return (
    <>
      <div
        ...
        onClick={handleClick}
        style={{ padding:18, display:'flex', flexDirection:'column', gap:10, cursor:isDragging?'grabbing':'pointer', opacity:isDragging?0.5:1, transform:isDragging?'rotate(1.5deg) scale(1.02)':'none', transition:'opacity .15s, transform .15s', animation:'fadeUp .3s ease both', outline:selected?`2px solid ${T.amber}`:'none', outlineOffset:2 }}
      >
...
// ─── Notes home ───────────────────────────────────────────────────────────────
const NotesHome = ({ notes, onNewScan, onNoteSelect, onToggleFavorite, onDelete, onMassDelete, onAddFolder, onDeleteFolder, onRenameFolder, onMoveNote, onUpdateTags, folders }) => {
  ...
  const handleEnterSelectionMode = (id) => {
  ...
  const handleMassTrash = () => {
    onMassDelete([...selectedIds]);
    exitSelection();
    setMassTrashModal(false);
  };
  ...
  return (
    <div style={{ animation:'fadeUp .35s ease both' }}>
      ...
      {massTrashModal && createPortal(
        <MassTrashConfirmModal count={selectedIds.size} onConfirm={handleMassTrash} onCancel={() => setMassTrashModal(false)} />,
        document.body
      )}
    </div>
  );
  ...
  <div style={{ animation:'fadeUp .35s ease both' }}>
    ...
    {notes.length === 0 ? (
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
        {notes.map(note => (
          <div key={note.id} className="ud-card" style={{ padding:'16px 20px', display:'flex', alignItems:'center', gap:16, borderRadius:14, transition:'border-color .15s' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
              ...
              <button
                ...
                style={{ fontSize:12, padding:'6px 14px', borderColor:'rgba(248,113,113,.25)', color:T.red, display:'flex', alignItems:'center', gap:6 }}
              >
...
// ─── Favorites home ───────────────────────────────────────────────────────────
const FavoritesHome = ({ notes, folders, onNoteSelect, onToggleFavorite, onDelete, onUpdateTags, onMoveNote, onAddFolder, onNewScan }) => {
  const [search, setSearch] = useState('');
  ...
  return (
    <div style={{ fontFamily:T.font, animation:'fadeUp .35s ease both' }}>
      ...
      {favoriteNotes.length === 0 && (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'80px 0', gap:20 }}>
          ...
          <div style={{ textAlign:'center', maxWidth:300 }}>
            ...
            <p style={{ fontFamily:T.font, fontSize:13, color:T.muted, margin:'0 0 24px', lineHeight:1.6 }}>
              Star your most important notes to keep them at your fingertips. Open any note and click the star to favorite it.
            </p>
      ...
      {/* Grid — reuses NoteCard for consistent styling */}
      {filtered.length > 0 && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(260px, 1fr))', gap:12 }}>
          {filtered.map(n => (
            <NoteCard
...
const UserDashboard = ({ onLogout, onProcess, onNewScan, onNoteSelect, showUploadPage=false, showResultsPage=false, initialTab=null, onInitialTabConsumed }) => {
  ...
  const handleToggleFavorite = async (id) => {
    ...
    catch { setNotes(p => p.map(n => n.id===id ? {...n, favorite:!n.favorite} : n)); showToast('Failed to update favorite'); }
  };
  ...
  const handleDeleteNote = async (id) => {
    ...
    catch { setNotes(p => p.map(n => n.id===id ? {...n, deleted:false} : n)); showToast('Failed to move note to trash'); }
  };

  const handleMassDelete = async (ids) => {
    setNotes(p => p.map(n => ids.includes(n.id) ? {...n, deleted:true, favorite:false} : n));
    showToast(`${ids.length} note${ids.length!==1?'s':''} moved to trash`);
    try { await Promise.all(ids.map(id => api.trashNote(id))); }
    catch { setNotes(p => p.map(n => ids.includes(n.id) ? {...n, deleted:false} : n)); showToast('Failed to move some notes to trash'); }
  };

  const handleRestoreNote = async (id) => {
    setNotes(p => p.map(n => n.id===id ? {...n, deleted:false} : n));
    try { await api.restoreNote(id); }
    catch { setNotes(p => p.map(n => n.id===id ? {...n, deleted:true} : n)); showToast('Failed to restore note'); }
  };
  ...
  const handlePermanentDelete = async (id) => {
    ...
    catch { if (backup) setNotes(p => [...p, backup]); showToast('Failed to delete note'); }
  };
  ...
  const handleEmptyTrash = async () => {
    ...
    catch { setNotes(p => [...p, ...trashed]); showToast('Failed to empty trash'); }
  };
  ...
  const handleUpdateTags = async (id, tags) => {
    ...
    catch { if (prev) setNotes(p => p.map(n => n.id===id ? {...n, tags:prev} : n)); showToast('Failed to update tags'); }
  };
  ...
  const handleMoveNote = async (noteId, targetFolderId) => {
    ...
    catch { setNotes(p => p.map(n => n.id===noteId ? {...n, folderId:prev} : n)); showToast('Failed to move note'); }
  };
  ...
  const handleAddFolder = async (name) => {
    ...
    } catch { setFolders(p => p.filter(f => f.id!==tempFolder.id)); showToast('Failed to create folder'); }
  };
  ...
  const handleDeleteFolder = async (fid) => {
    ...
    catch { if (backup) setFolders(p => [...p, backup]); showToast('Failed to delete folder'); }
  };
  ...
  const handleRenameFolder = async (id, name) => {
    ...
    catch { if (prev) setFolders(p => p.map(f => f.id===id ? {...f, name:prev} : f)); showToast('Failed to rename folder'); }
  };
  ...
  return (
    <DragContext.Provider value={{ draggingNoteId, setDraggingNoteId }}>
        ...
        <main className="ud-scrollbar" style={{ flex:1, overflowY:'auto', height:'100vh', boxSizing:'border-box' }}>
          ...
          {activeTab !== 'results' && (
            <div style={{ padding:'32px 40px', maxWidth:1100, margin:'0 auto' }}>
              {activeTab === 'notes'     && <NotesHome notes={activeNotes} folders={folders} onNewScan={handleNewScan} onNoteSelect={onNoteSelect} onToggleFavorite={handleToggleFavorite} onDelete={handleDeleteNote} onMassDelete={handleMassDelete} onAddFolder={() => setShowNewFolder(true)} onDeleteFolder={handleDeleteFolder} onRenameFolder={handleRenameFolder} onMoveNote={handleMoveNote} onUpdateTags={handleUpdateTags} />}
              {activeTab === 'upload'    && <UploadPage onProcess={() => { setActiveTab('results'); if (onProcess) onProcess(); }} />}
              {activeTab === 'favorites' && <FavoritesHome notes={activeNotes} folders={folders} onNoteSelect={onNoteSelect} onToggleFavorite={handleToggleFavorite} onDelete={handleDeleteNote} onUpdateTags={handleUpdateTags} onMoveNote={handleMoveNote} onAddFolder={() => setShowNewFolder(true)} onNewScan={handleNewScan} />}
      ...
      <div className={`ud-toast${toast.visible?' visible':''}`}><Icon d="M5 13l4 4L19 7" size={13} color={T.green} />{toast.message}</div>
