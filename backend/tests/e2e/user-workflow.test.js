...
describe('End-to-end user workflow', () => {
  ...
  test('complete student workflow: register, login, upload, organize, edit, trash, delete', async () => {
    ...
    expect(profileRes.body.email).toBe('jane.e2e@example.com');
    ...
    expect(titleRes.body.tags).toEqual(['biology', 'cells']);
    ...
    const editTranscriptRes = await request(app)
      .put(`/api/files/${fileId}/edit/transcription`)
      .set('x-auth-token', token)
      .send({ previousText: '', newText: 'Mitochondria are the powerhouse of the cell.' });
    expect(editTranscriptRes.status).toBe(200);
    expect(editTranscriptRes.body.editCount).toBe(1);

    const editSummaryRes = await request(app)
      .put(`/api/files/${fileId}/edit/summary`)
      .set('x-auth-token', token)
      .send({ previousText: '', newText: 'Overview of cell components.' });
    expect(editSummaryRes.body.currentContent.summary).toBe('Overview of cell components.');

    const historyRes = await request(app)
      .get(`/api/files/${fileId}/edits`)
      .set('x-auth-token', token);
    expect(historyRes.body.transcriptionEdits).toHaveLength(1);
    expect(historyRes.body.summaryEdits).toHaveLength(1);
    ...
    const mainListAfterTrash = await request(app)
      .get('/api/files')
      .set('x-auth-token', token);
  ...
  test('login flow with pre-existing account preserves uploaded files', async () => {
    const register = await request(app)
      .post('/api/auth/register')
      .send({ fullName: 'Return User', email: 'return@example.com', password: 'password123' });
    ...
    const login = await request(app)
      .post('/api/auth/login')
      .send({ email: 'return@example.com', password: 'password123' });
  ...
  test('mock credentials flow from LoginPage.jsx works end-to-end', async () => {
    const login = await request(app)
      .post('/api/auth/login')
