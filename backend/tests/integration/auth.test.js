...
describe('Auth API integration', () => {
  ...
  describe('POST /api/auth/register', () => {
    test('creates a new user with valid data', async () => {
      const res = await request(app).post('/api/auth/register').send({
        fullName: 'New User', email: 'newuser@example.com', password: 'securepass123',
      });
      expect(res.status).toBe(201);
      expect(res.body).toHaveProperty('token');
      expect(res.body.user).toMatchObject({ email: 'newuser@example.com', fullName: 'New User' });
    });

    test('hashes the password before storing', async () => {
      await request(app).post('/api/auth/register').send({
        fullName: 'Secure', email: 'hashed@example.com', password: 'plainpassword',
      });
    ...
    test('rejects duplicate email', async () => {
      ...
      const res = await request(app).post('/api/auth/register').send({ email: 'dup@example.com', password: 'anotherpass' });
      expect(res.status).toBe(400);
    ...
    test('rejects duplicate email regardless of case', async () => {
      ...
      const res = await request(app).post('/api/auth/register').send({ email: 'CASE@Example.com', password: 'password456' });
      expect(res.status).toBe(400);
    });
  });
  ...
  describe('POST /api/auth/login', () => {
    ...
    test('rejects login with missing password', async () => {
      const res = await request(app).post('/api/auth/login').send({ email: 'real@example.com' });
      expect(res.status).toBe(400);
    });
  ...
  describe('GET /api/auth/me', () => {
    ...
    beforeEach(async () => {
      const res = await request(app).post('/api/auth/register').send({
        fullName: 'Profile User', email: 'profile@example.com', password: 'password123',
      });
